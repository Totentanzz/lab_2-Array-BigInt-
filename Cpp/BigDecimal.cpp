...
* 1) принимает все цифры слева до '.' за целую часть, записывая их в массив
*	 следующим образом: 
     сначала записывается кол-во цифр, которое равно остатку от деления на 9,
*	 затем записываются оставшиеся цифры группами по 9
* 2) после '.' все оставшиеся цифры представляют собой дробную часть, которая
*	 записывается следующим образом: берется число, состоящее из 9 цифр (если
*    цифр осталось меньше 9, то берутся все оставшиеся), подсчитвается кол-во
*    цифр curDigits, число умножается на 10^N, где N=9-curDigits, и
*    записывается в массив (умножение на 10^N необходимо, чтобы учитывать
...
BigDecimal::BigDecimal(char* valStr) {
	...
	if (end == isDot)
		throw std::invalid_argument("BigDecimal constructor: dot is not found");
...
/*
* Конструктор, принимающий в качестве аргумента число (вызывает конструктор
* из строки)
*/
...
void BigDecimal::setSign(short newSign) {
	if (newSign < -1 || newSign == 0 || newSign >> 1)
		throw std::invalid_argument("setSign: invalid sign");
	sign = newSign;
}
...
*	 встреченной ',' (т.е. если кол-во пройденных элементов массива равно
     кол-ву целых элементов intCells в массиве)
* 5) в случае если целая часть закончилась и '.' уже присутствует, то для
     каждого следующего элемента, представляющего собой нецелую часть,
	 подсчитывает кол-во нулей слева, смещает элемент, добавляет нули слева
...
* 2)возвращает  1, если целая часть первого > целой части второго
*  (intCells_1>intCells_2)
* 3)возвращает  0, если равны
* 4)возвращает -1, если целая часть первого < целой части второго
*   (intCells_1<intCells_2)
...
int BigDecimal::getStrLen(char* str) {
	...
	for (i = 0; str[i] != '\0'; i++) {
		if ((str[i] < 48 && str[i] != 46) || str[i] > 57)
			throw std::invalid_argument("getStrLen: invalid string");
	}
	return i;
}
...
int BigDecimal::powOfTen(int deg) {
	if (deg < 0)
		throw DataException("powOfTen: invalid degree", deg);
	int result = 1;
	while (deg) {
		result *= 10;
		deg--;
